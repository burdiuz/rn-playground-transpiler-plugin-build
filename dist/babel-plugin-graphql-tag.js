...
var babelLiteralToAst = _interopDefault(require('babel-literal-to-ast'));
var graphqlTag = _interopDefault(require('graphql-tag'));
var tty = _interopDefault(require('tty'));
var util = _interopDefault(require('util'));
var os = _interopDefault(require('os'));
var graphql = _interopDefault(require('graphql'));
...
var _babelLiteralToAst = _interopRequireDefault(babelLiteralToAst);

var _graphqlTag = _interopRequireDefault(graphqlTag);
...
const {
  ...
  callExpression,
  identifier
} = core.types; // eslint-disable-next-line no-restricted-syntax

const uniqueFn = (0, parser.parseExpression)(`
  (definitions) => {
    const names = {};
...
var _default = (0, helperPluginUtils.declare)((api, options) => {
  ...
  const compile = (path, uniqueId) => {
    ...
    const queryDocument = (0, _graphqlTag.default)(strip ? (0, graphql.stripIgnoredCharacters)(source) : source); // If a document contains only one operation, that operation may be unnamed:
...
var graphQLTag = unwrapExports(dist);

var getRNPlaygroundPlugin=function getRNPlaygroundPlugin(){return graphQLTag;};var getRNPlaygroundPluginConfig=function getRNPlaygroundPluginConfig(config){return [graphQLTag,config];};
